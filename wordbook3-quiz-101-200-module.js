// V502 Synonym Drill — Wordbook3 Quiz 101-200 (Integrated Panel)
var WB3_QUIZ_2 = WB3_QUIZ_101_200;

(function() {
  var LETTERS = ['A','B','C','D','E'];
  var SESSION_KEY = "v502-synonym-drill-session";
  var PROGRESS_KEY = "v502-synonym-drill-wb3quiz2-progress";
  var SCORE_KEY = "v502-synonym-drill-wb3quiz2-best";
  ...
  function render() {
    ...
    navHtml += '<button class="wb3q-btn sec" onclick="wb3Prev2()"' + (cur === 0 ? ' disabled style="opacity:0.4"' : '') + '>◀ 이전</button>';
    navHtml += '<span style="flex:1;text-align:center;font-size:0.8rem;color:#94a3b8;align-self:center">' + (cur+1) + ' / 100</span>';
    navHtml += '<button class="wb3q-btn sec" onclick="wb3Next2()"' + (cur === 99 ? ' disabled style="opacity:0.4"' : '') + '>다음 ▶</button>';
    navHtml += '</div>';
  ...
  window.wb3Pick2 = function(btn, chosen) {
    var q = WB3_QUIZ_2[cur];
  ...
  window.wb3SubmitNext2 = function() {
    do { cur++; } while (cur < 100 && answered[cur+1] && picks[cur+1] === WB3_QUIZ_2[cur].a);
    if (cur >= 100) { showResult(); return; }
    render();
  };

  window.wb3Prev2 = function() { if (cur > 0) cur--; render(); };
  window.wb3Next2 = function() { if (cur < 99) cur++; else { cur = 100; showResult(); return; } render(); };
  ...
  function showResult() {
    ...
    h += '<button class="wb3q-btn" onclick="wb3Restart2()">🔄 다시 풀기</button>';
    h += '<button class="wb3q-btn sec" onclick="wb3Reset2()">🗑️ 초기화</button>';
    h += '</div>';
    area.innerHTML = h;
  ...
  window.wb3Restart2 = function() {
    cur = 0; while (cur < 100 && answered[cur+1] && picks[cur+1] === WB3_QUIZ_2[cur].a) cur++;
    if (cur >= 100) showResult(); else render();
  };

  window.wb3Reset2 = function() {
    if (!confirm('101-200번 진행상황과 점수를 초기화하시겠습니까?')) return;
    answered = {}; picks = {}; scored = {}; score = 0; cur = 0; save();
    WB3_QUIZ_2.forEach(function(q){ for(var i=q.o.length-1;i>0;i-- ){ var j=Math.floor(Math.random()*(i+1)),t=q.o[i];q.o[i]=q.o[j];q.o[j]=t; } });
    render();
  };
