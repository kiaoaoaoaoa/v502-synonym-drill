var WB3_QUIZ_1000 = WB3_QUIZ_1000_1099;

(function() {
  var LETTERS = ['A','B','C','D','E'];
  var SESSION_KEY = "v502-synonym-drill-session";
  var PROGRESS_KEY = "v502-synonym-drill-wb3quiz3-progress";
  var SCORE_KEY = "v502-synonym-drill-wb3quiz3-best";
  ...
  function render() {
    ...
    navHtml += '<button class="wb3q-btn sec" onclick="wb3Prev3()"' + (cur === 0 ? ' disabled style="opacity:0.4"' : '') + '>◀ 이전</button>';
    navHtml += '<span style="flex:1;text-align:center;font-size:0.8rem;color:#94a3b8;align-self:center">' + (cur+1) + ' / 100</span>';
    navHtml += '<button class="wb3q-btn sec" onclick="wb3Next3()"' + (cur === 99 ? ' disabled style="opacity:0.4"' : '') + '>다음 ▶</button>';
    navHtml += '</div>';
    ...
    if (wasCorrect) {
      ...
      if (typeof EXPLANATIONS_1000_1099 !== 'undefined' && EXPLANATIONS_1000_1099[q.n]) {
        h += '<span class="wb3q-exp-toggle" onclick="var e=document.getElementById(\'wb3exp3-'+q.n+'\');e.classList.toggle(\'show\');this.textContent=e.classList.contains(\'show\')?\'📖 해설 접기\':\'📖 해설 보기\'">📖 해설 보기</span>';
        h += '<div class="wb3q-exp" id="wb3exp3-' + q.n + '">' + EXPLANATIONS_1000_1099[q.n] + '</div>';
      }
      h += navHtml;
    } else {
      if (wasWrong) h += '<div class="wb3q-fb show no" style="margin-bottom:10px">이전: <strong>' + picks[cur+1] + '</strong> (오답). 다시 풀어보세요.</div>';
      h += '<div class="wb3q-opts">';
      q.o.forEach(function(o,i) { h += '<button class="wb3q-opt" onclick="wb3Pick3(this,\'' + o.replace(/'/g,"\\'") + '\')" data-opt="' + o.replace(/"/g,'&quot;') + '"><span class="wb3q-letter">' + LETTERS[i] + '</span>' + o + '</button>'; });
      h += '</div><div class="wb3q-fb" id="wb3fb3"></div><div class="wb3q-exp" id="wb3exp3-'+q.n+'"></div>';
      h += '<span class="wb3q-exp-toggle" id="wb3tgl3-'+q.n+'" style="display:none" onclick="var e=document.getElementById(\'wb3exp3-'+q.n+'\');e.classList.toggle(\'show\');this.textContent=e.classList.contains(\'show\')?\'📖 해설 접기\':\'📖 해설 보기\'">📖 해설 보기</span>';
      h += navHtml;
    }
    h += '</div>';
  ...
  window.wb3Pick3 = function(btn, chosen) {
    var q = WB3_QUIZ_1000[cur];
    ...
    var fb = document.getElementById('wb3fb3'); if (fb) { fb.classList.add('show'); fb.classList.add(ok?'ok':'no'); fb.textContent = ok ? '✅ 정답입니다!' : '❌ 오답입니다. 정답: ' + q.a; }

    if (typeof EXPLANATIONS_1000_1099 !== 'undefined' && EXPLANATIONS_1000_1099[q.n]) {
      var ed = document.getElementById('wb3exp3-'+q.n); if (ed) { ed.innerHTML = EXPLANATIONS_1000_1099[q.n]; ed.classList.add('show'); }
      var et = document.getElementById('wb3tgl3-'+q.n); if (et) { et.style.display = 'inline-block'; et.textContent = '📖 해설 접기'; }
    }

    if (isNoExplain) { setTimeout(function() { wb3SubmitNext3(); }, 400); }
  };

  window.wb3SubmitNext3 = function() {
    do { cur++; } while (cur < 100 && answered[cur+1] && picks[cur+1] === WB3_QUIZ_1000[cur].a);
    if (cur >= 100) { showResult(); return; }
    render();
  };

  window.wb3Prev3 = function() { if (cur > 0) cur--; render(); };
  window.wb3Next3 = function() { if (cur < 99) cur++; else { cur = 100; showResult(); return; } render(); };
  ...
  function showResult() {
    ...
    h += '<button class="wb3q-btn" onclick="wb3Restart3()">🔄 다시 풀기</button>';
    h += '<button class="wb3q-btn sec" onclick="wb3Reset3()">🗑️ 초기화</button>';
    h += '</div>';
    area.innerHTML = h;
  ...
  window.wb3Restart3 = function() {
    cur = 0; while (cur < 100 && answered[cur+1] && picks[cur+1] === WB3_QUIZ_1000[cur].a) cur++;
    if (cur >= 100) showResult(); else render();
  };

  window.wb3Reset3 = function() {
    if (!confirm('1000-1099번 진행상황과 점수를 초기화하시겠습니까?')) return;
    answered = {}; picks = {}; scored = {}; score = 0; cur = 0; save();
    WB3_QUIZ_1000.forEach(function(q){ for(var i=q.o.length-1;i>0;i-- ){ var j=Math.floor(Math.random()*(i+1)),t=q.o[i];q.o[i]=q.o[j];q.o[j]=t; } });
    render();
  };
