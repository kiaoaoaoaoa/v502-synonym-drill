...
function showWordbook3() {
  ...
  sectionItems.forEach((sec, idx) => {
    ...
    var label = (sec.start + 1) + '~' + (sec.end + 1) + '번 단어';
    html += '<details class="wb3-section"' + (idx === 0 ? ' open' : '') + '>';
    ...
    if (idx !== 10 && typeof window['showWb3QuizR' + (idx * 100)] === 'function') {
      ...
      html += ' <button onclick="showWb3QuizR' + rangeStart + '()" class="wb3-quiz-btn" style="margin-left:8px;font-size:0.78rem;padding:2px 10px;border-radius:99px;background:#4c6ef5;color:#fff;border:none;cursor:pointer;font-weight:600;white-space:nowrap">📝 단어문제 ' + (rangeStart + 1) + '-' + (rangeStart + 100) + '</button>';
    }
    if (idx === 10) {
  ...
  if (tab === 'review') {
    ...
    if (chapters.length === 0) {
      ...
      chapters.forEach(function(ch, i) {
        ...
        html += '<summary class="wb3-summary" style="font-size:13px;font-weight:700;cursor:pointer;padding:8px 12px;background:#f8f9fc;border-radius:4px;border:1px solid var(--line)">📖 ' + escapeHtml(ch.title) + '</summary>';
        html += '<div style="padding:12px 16px;font-size:13px;line-height:1.8;white-space:pre-wrap;background:#fff;border:1px solid var(--line);border-top:none;border-radius:0 0 4px 4px;max-height:60vh;overflow-y:auto">' + bodyHtml + '</div>';
        html += '</details>';
      });
    }
  }
...
var examHideCorrect = false; // hide correctly answered questions
...
var EXAM_REGISTRY = {
  ...
  hanyang2024:    { title: '2024 한양대 서울 인문',   data: function(){return window.__V502_EXAM_HANYANG2024__||[]} }
};
...
function renderExamTab() {
  ...
  // Filter correct if hide mode on
  var visibleQuestions = questions;
  var visibleToOrig = null; // maps visibleIdx → originalIdx
  if (examHideCorrect) {
    visibleToOrig = [];
    visibleQuestions = [];
    questions.forEach(function(q, i) {
      if (!prog.correct.has(i)) { visibleToOrig.push(i); visibleQuestions.push(q); }
    });
  }
  function origIdx(vi) { return visibleToOrig ? visibleToOrig[vi] : vi; }

  // Clamp examIndex
  if (examIndex >= visibleQuestions.length) examIndex = Math.max(0, visibleQuestions.length - 1);
  if (examIndex < 0) examIndex = 0;

  // --- Top nav bar ---
  html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;flex-wrap:wrap">';
  html += '<button onclick="examHideCorrect=!examHideCorrect;examIndex=0;renderExamTab()" style="padding:4px 10px;border:1px solid var(--line);border-radius:4px;font-size:12px;cursor:pointer;background:' + (examHideCorrect?'var(--ok)':'transparent') + ';color:' + (examHideCorrect?'#fff':'var(--ink)') + ';white-space:nowrap">👁 정답숨김</button>';
  ...
  var vLen = visibleQuestions.length;
  if (examOneByOne && vLen > 1) {
    ...
    html += '<span style="font-size:13px;font-weight:700">' + (examIndex+1) + ' / ' + vLen + '</span>';
    html += '<button onclick="examIndex=Math.min(' + (vLen-1) + ',examIndex+1);renderExamTab()" style="padding:4px 8px;border:1px solid var(--line);border-radius:4px;font-size:12px;cursor:pointer;background:transparent;color:var(--ink)"' + (examIndex>=vLen-1?' disabled':'') + '>▶</button>';
    html += '<button onclick="examIndex=' + (vLen-1) + ';renderExamTab()" style="padding:4px 8px;border:1px solid var(--line);border-radius:4px;font-size:12px;cursor:pointer;background:transparent;color:var(--ink)"' + (examIndex>=vLen-1?' disabled':'') + '>⏭</button>';
    html += '</div>';
  }
  html += '</div>';
  ...
  if (examOneByOne && vLen > 1) {
    ...
    html += '<span style="font-size:14px;font-weight:700;color:var(--accent)">' + (examIndex+1) + ' / ' + vLen + '</span>';
    html += '<button onclick="examIndex=Math.min(' + (vLen-1) + ',examIndex+1);renderExamTab()" style="padding:6px 16px;border:1px solid var(--line);border-radius:4px;font-size:14px;cursor:pointer;background:var(--accent);color:#fff"' + (examIndex>=vLen-1?' disabled':'') + '>다음 ▶</button>';
    html += '</div>';
  }
