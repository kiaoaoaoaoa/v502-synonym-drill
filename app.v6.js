...
async function completeQuiz() {
  ...
  els.resultSummary.innerHTML = `${entry.correct}문제 풀었어요 — 정답률 ${accuracy}% — ${tier.icon} ${tier.name}`;
...
function renderPublicResult(displayName, accuracy, correct, total) {
  ...
  getSupabaseClient().then((client) => {
    ...
    readPublicLeaderboard().then((data) => {
      ...
      els.resultSummary.textContent = `${correct}문제 — 정답률 ${accuracy}% — Public Rank #${rank || cumulative.length + 1}`;
...
function showLogicAllDone() {
  ...
  els.resultSummary.textContent = `모든 문제 완료! ${correct}문제 정답 (정답률 ${pct}%) · 난이도 가중 점수: ${ws.score.toFixed(2)} (${ws.questions}문제)`;
}
...
function finishLogicQuiz() {
  ...
  els.resultPanel.hidden = false;
  els.resultTitle.textContent = "Logic Quiz Result";
  els.resultSummary.textContent = `이번 세션 ${logicState.correctCount}문제 정답 (정답률 ${sessionPct}%) · 누적 ${cumCorrect}문제 정답 (정답률 ${cumPct}%) · 가중점수 ${ws.score.toFixed(2)} · 남은 ${remaining}문제`;
}
...
function showDashboard() {
  ...
  if (loggedIn) {
    ...
    html += `<div class="dash-stat" onclick="startLogicQuiz()" style="cursor:pointer" title="논리문제"><span class="dash-stat-num">${logicMastered}</span><span class="dash-stat-label">${icon('puzzle')} 논리 마스터</span></div>`;
...
function showMyReview() {
  ...
  html += `<div style="padding:16px;background:#f0f8f0;border-radius:8px;text-align:center"><strong style="font-size:24px">${completed}</strong><br><small>단어 마스터</small><br><small style="color:var(--muted)">정답률 ${pct}%</small></div>`;
  html += `<div style="padding:16px;background:#f0f0f8;border-radius:8px;text-align:center"><strong style="font-size:24px">${logicMastered}</strong><br><small>논리 마스터</small><br><small style="color:var(--muted)">정답률 ${logicTotal > 0 ? Math.round((logicMastered/logicTotal)*100) + '%' : 'No data'}</small></div>`;
  html += `<div style="padding:16px;background:#e8f0e8;border-radius:8px;text-align:center"><strong style="font-size:24px">${wcProgress.correct}</strong><br><small>단어확인</small><br><small style="color:var(--muted)">정답률 ${wcProgress.total > 0 ? Math.round(wcProgress.correct/wcProgress.total*100) + '%' : 'No data'}</small></div>`;
  html += `<div style="padding:16px;background:#fff8f0;border-radius:8px;text-align:center"><strong style="font-size:24px">${totalCorrect}</strong><br><small>통합랭킹 점수</small><br><small style="color:var(--muted)">정답률 ${totalQuestions > 0 ? Math.round((totalCorrect/totalQuestions)*100) + '%' : 'No data'}</small></div>`;
  html += '</div>';
...
function renderMyInfoTab(tab) {
  ...
  if (tab === 'review') {
    ...
    html += `<div style="padding:16px;border:1px solid var(--line);border-left:3px solid var(--ok);border-radius:2px;text-align:center"><strong style="font-size:24px">${completed}</strong><br><small>단어 마스터</small><br><small style="color:var(--muted)">정답률 ${pct}%</small></div>`;
    html += `<div style="padding:16px;border:1px solid var(--line);border-left:3px solid #4f46e5;border-radius:2px;text-align:center"><strong style="font-size:24px">${logicMastered}</strong><br><small>논리 마스터</small><br><small style="color:var(--muted)">${logicTotal > 0 ? Math.round((logicMastered/logicTotal)*100) + '%' : 'No data'}</small></div>`;
    html += `<div style="padding:16px;border:1px solid var(--line);border-left:3px solid var(--ink);border-radius:2px;text-align:center"><strong style="font-size:24px">${wcProgress.correct}</strong><br><small>단어확인</small><br><small style="color:var(--muted)">정답률 ${wcProgress.total > 0 ? Math.round(wcProgress.correct/wcProgress.total*100) + '%' : 'No data'}</small></div>`;
    html += `<div style="padding:16px;border:1px solid var(--line);border-left:3px solid var(--warn);border-radius:2px;text-align:center;cursor:pointer" onclick="showRanking()" title="통합랭킹 보기"><strong style="font-size:24px">${totalCorrect}</strong><br><small>통합랭킹 점수 ▸</small><br><small style="color:var(--muted)">정답률 ${totalQuestions > 0 ? Math.round((totalCorrect/totalQuestions)*100) + '%' : 'No data'}</small></div>`;
    html += '</div>';
...
function finishGrammarQuiz() {
  ...
  els.grammar201Content.innerHTML = `<div style="text-align:center;padding:40px"><h3>문법 201 완료</h3><p>${grammarState.correct}문제 정답 (정답률 ${pct}%)</p><button onclick="showGrammar201()" class="text-btn" style="min-height:40px;margin-top:12px">다시 보기</button></div>`;
}
